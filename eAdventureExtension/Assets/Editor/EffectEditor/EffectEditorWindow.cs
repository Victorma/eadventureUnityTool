...
public class EffectEditorWindow: EditorWindow{
    ...
    [MenuItem("eAdventure/Open Effect editor")]
    ...
    void createWindow(AbstractEffect effect){
        if (editors.ContainsKey (effect) )
            editors[effect].Window = GUILayout.Window(windowId, editors[effect].Window, nodeWindow, effect.getType().ToString(), GUILayout.MinWidth(150));
        else
            tmpRects[effect] = GUILayout.Window(windowId, tmpRects[effect], nodeWindow, effect.getType().ToString(), GUILayout.MinWidth(150));
    }
    ...
    void OnGUI()
    {
        ...
        if(GUILayout.Button("New Effect")){
            effects.add (new ActivateEffect("Test"));
        }
