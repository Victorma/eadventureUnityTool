...
public class ConversationEditorWindow : EditorWindow{
	...
	void nodeWindow(int id)
	{
		...
		{
            GUIContent bttext = new GUIContent("Open");
        ...
        {
            if (new Rect(0, 0, editors[myNode].Window.width, editors[myNode].Window.height).Contains(Event.current.mousePosition))
            {
                hovering = id;
            }
        }
                    ...
                    { 
                        ...
                        if (lookingChildNode == n && lookingChildSlot == i) {
                            ...
                            if (hovering != -1) 
                                ...
                                curveFromTo(fromRect, new Rect(Event.current.mousePosition.x, Event.current.mousePosition.y, 1, 1), Color.blue, s);
                        }else
	...
	void OnGUI()
	{
        ...
        conversation.setId(EditorGUILayout.TextField("Conversation ID: ",conversation.getId(),GUILayout.ExpandWidth(true)));
        GUILayout.EndVertical();
