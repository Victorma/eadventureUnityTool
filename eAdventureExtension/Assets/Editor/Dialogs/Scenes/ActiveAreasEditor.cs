...
public class ActiveAreasEditor : BaseAreaEditablePopup
{
    ...
    public void Init(DialogReceiverInterface e, SceneDataControl scene, int areaIndex)
    {
        ...
        widthString = widthStringLast = sceneRef.getActiveAreasList().getActiveAreasList()[areaIndex].getWidth().ToString();
        heightString = heightStringLast = sceneRef.getActiveAreasList().getActiveAreasList()[areaIndex].getHeight().ToString();
    ...
    void OnGUI()
    {
        ...
        for (int i = 0;
            ...
            i++)
        {
            Rect eRect = new Rect(sceneRef.getExitsList().getExitsList()[i].getX(), sceneRef.getExitsList().getExitsList()[i].getY(), sceneRef.getExitsList().getExitsList()[i].getWidth(), sceneRef.getExitsList().getExitsList()[i].getHeight());
    ...
    {
        xStringLast = val;
        sceneRef.getActiveAreasList().getActiveAreasList()[calledAreaIndexRef].setValues(int.Parse(xString), int.Parse(yString), int.Parse(widthString), int.Parse(heightString));
    }
